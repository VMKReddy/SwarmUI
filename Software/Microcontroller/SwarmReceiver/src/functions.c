...
void assignNewRobot()
{
    // id of robots starts with 0, and is incremented for each that's connected
    // pipAddress starts with 0xF0F0F0F0D000+2,
    ...
    if(myRobots.count <= NB_MAX_ROBOTS)
    {
        ...
        // copy the assigned pipeAddress into the payload of the message to send to the Zooid
        ...
        // adds new robot to receiver's robot buffer
        RobotRingBuffer_push(&myRobots, newComingRobot);
