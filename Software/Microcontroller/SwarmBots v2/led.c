...
void setRGBLed(uint8_t red, uint8_t green, uint8_t blue)
{
    if (isRGBInitialized)
    {
        ...
        __HAL_TIM_SET_COMPARE(&rgbTimHandle, RED_CHANNEL, currentRed);
        __HAL_TIM_SET_COMPARE(&rgbTimHandle, GREEN_CHANNEL, currentGreen);
        __HAL_TIM_SET_COMPARE(&rgbTimHandle, BLUE_CHANNEL, currentBlue);
    }
}
...
void setRedLedRaw(uint16_t redRaw)
{
    if (redRaw <= RGB_PERIOD && isRGBInitialized)
        __HAL_TIM_SET_COMPARE(&rgbTimHandle, RED_CHANNEL, redRaw);
}
...
void setGreenLedRaw(uint16_t greenRaw)
{
    if (greenRaw <= RGB_PERIOD && isRGBInitialized)
        __HAL_TIM_SET_COMPARE(&rgbTimHandle, GREEN_CHANNEL, greenRaw);
}
...
void setBlueLedRaw(uint16_t blueRaw)
{
    if (blueRaw <= RGB_PERIOD && isRGBInitialized)
        __HAL_TIM_SET_COMPARE(&rgbTimHandle, BLUE_CHANNEL, blueRaw);
}
